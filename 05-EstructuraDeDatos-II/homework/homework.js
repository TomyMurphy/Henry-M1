...
LinkedList.prototype.search = function(value) {
  ...
  while (current) {
    if (typeof value === "string" && current.value === value) return current.value;
    if (typeof value === "function" && value(current.value)) return current.value;
...
La clase debe tener los siguientes métodos:
  - hash: función hasheadora que determina en qué bucket se almacenará un dato. Recibe un input alfabético, suma el código numérico
    de cada caracter del input (investigar el método charCodeAt de los strings) y calcula el módulo de ese número total
